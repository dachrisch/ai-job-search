...
describe('SSEManager', () => {
  ...
  it('should subscribe a client to a search', () => {
    ...
    expect(manager.getConnectedClientCount('search123')).toBe(1)
  })
  ...
  it('should broadcast to all clients for a search', () => {
    const res1 = { write: vi.fn(), on: vi.fn(), once: vi.fn() } as any
    const res2 = { write: vi.fn(), on: vi.fn(), once: vi.fn() } as any
  ...
  it('should format messages as Server-Sent Events', () => {
    const res = { write: vi.fn(), on: vi.fn(), once: vi.fn() } as any
  ...
  it('should support multiple subscriptions by same client', () => {
    const res = { write: vi.fn(), on: vi.fn(), once: vi.fn() } as any
    manager.subscribe('search123', res)
    manager.subscribe('search456', res)

    expect(manager.getConnectedClientCount('search123')).toBe(1)
    expect(manager.getConnectedClientCount('search456')).toBe(1)
  })

  it('should handle broadcast errors and unsubscribe on failure', () => {
    const res = { write: vi.fn(() => {
      throw new Error('Write failed')
    }), on: vi.fn(), once: vi.fn() } as any
    ...
    expect(manager.getConnectedClientCount('search123')).toBe(0)
  })
})
